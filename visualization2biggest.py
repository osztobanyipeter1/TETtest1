...
def main():
    ...
    # A 2 legnagyobb klaszter kiválasztása
    unique_labels = np.unique(labels)
    ...
    # A 2 legnagyobb klaszter kiválasztása (ha van legalább 2)
    if len(cluster_sizes) >= 2:
        top_2_clusters = [cluster_sizes[0][0], cluster_sizes[1][0]]
        print(f"A 2 legnagyobb klaszter: {top_2_clusters} (méretük: {cluster_sizes[0][1]}, {cluster_sizes[1][1]})")
    else:
        top_2_clusters = [cluster_sizes[0][0]] if cluster_sizes else []
        print(f"Csak {len(cluster_sizes)} klaszter van, az összes megjelenik.")

    kept_cloud, kept_labels = keep_only_selected_clusters(
        pcd,
        labels,
        selected_clusters=top_2_clusters if top_2_clusters else None
    )
