...
class ARPoseTrackingSystem:
    ...
    def __init__(self):
        ...
        self.scale_factor = 0.1  # Alapértelmezett skála
...
class PointCloudViewer:
    ...
    def initialize_tracking(self, gray_frame, color_frame):
        """
    ...
    def track_frame(self, gray_frame, color_frame):
        """
    ...
    def estimate_scale_and_update_position(self, motion_vector):
        """
        2. FÁZIS: Skála becslés és pozíció frissítés
        """
    ...
    def update_bundle_adjustment(self, prev_points, curr_points, motion):
        """
    ...
    def optimize_sliding_window(self):
        ...
        if len(self.ar_tracker.frame_poses) >= 3:
...
class PositionPlotter:
    def __init__(self):
        ...
        self.ax.set_xlabel('X (jobbra/balra)')
        self.ax.set_ylabel('Y (fel/le)')
        self.ax.set_zlabel('Z (előre/hátra)')
        self.ax.set_title('3D Pozíció és Orientáció')
    ...
    def draw(self):
        with self.lock:
            ...
            self.ax.plot(pos_array[:,0], pos_array[:,1], pos_array[:,2], 'b-')
            # Az utolsó orientációból kvaternió: jobbra (X, piros), fel (Y, zöld), előre (Z, kék)
            ori = self.orientations[-1]
            w, x, y, z = ori
            R = np.array([
                [1 - 2*(y**2 + z**2), 2*(x*y - z*w),     2*(x*z + y*w)],
                [2*(x*y + z*w),     1 - 2*(x**2 + z**2), 2*(y*z - x*w)],
            ...
            self.ax.quiver(*origin, *(R[:,0]*scale), color='r')
            self.ax.quiver(*origin, *(R[:,1]*scale), color='g')
            self.ax.quiver(*origin, *(R[:,2]*scale), color='b')
            plt.draw()
            plt.pause(0.001)

def plotter_thread(viewer, plotter):
    # Folyamatosan olvassa az aktuális pozíciót és orientációt
    while getattr(viewer, "running", False):
        with viewer.lock:
            pos = viewer.ar_tracker.position_3d.copy()
            ori = [
                viewer.quaternion_w,
                viewer.quaternion_x,
                viewer.quaternion_y,
...
if __name__ == "__main__":
    try:
        ...
        t = threading.Thread(target=plotter_thread, args=(viewer, plotter), daemon=True)
        t.start()
        viewer.run()
    except Exception as e:
        print(f"Hiba: {e}")
