                        ...
                        try:
                            ...
                            with self.lock:
                                if "position" in pos_data:
                                    self.camera_pos = np.array(pos_data["position"], dtype=np.float32)
                                if "quaternion" in pos_data:
                                    q = pos_data["quaternion"]
                                    self.quaternion_w, self.quaternion_x, self.quaternion_y, self.quaternion_z = map(float, q)

                        except json.JSONDecodeError:
                            print("Hibás adat formátum")
