...
class PointCloudViewer:
    ...
    def get_visible_points(self):
        ...
        vertices_gpu = cp.asarray(self.vertices) # GPU-ra másolás
        camera_pos_gpu = cp.asarray(self.camera_pos)
        camera_front_gpu = cp.asarray(self.camera_front)

        directions = vertices_gpu - camera_pos_gpu
        distances = cp.linalg.norm(directions, axis=1)
        directions_normalized = directions / distances[:, cp.newaxis]

        dot = cp.dot(directions_normalized, camera_front_gpu)
        mask = (distances < self.max_distance) & (dot > self.fov_cos)

       
        visible_vertices = cp.asnumpy(vertices_gpu[mask]) # CPU-ra másolás
        visible_colors = self.colors[cp.asnumpy(mask)]
...
if __name__ == "__main__":
    ...
    - ESC: Kilépés
    """)
    viewer = PointCloudViewer("cave_sampled2.ply")
    viewer.run()
